...
\newcommand{\PthreePop}{43{,}732{,}020}    % P3 popularity-policy travel
\newcommand{\PthreePopSave}{7.4}    % P3 % saving vs popularity policy
\newcommand{\PtwoBuildAll}{1{,}033{,}711}  % P2 build-everywhere cost
\newcommand{\PtwoBuildSave}{15.1}   % P2 % saving vs build-everywhere
\newcommand{\PoneGLB}{3{,}199{,}175}       % P1 full GL lower bound
\newcommand{\PoneGLBgap}{54.1}      % SA gap to GL bound (loose)
\newcommand{\PoneRandImp}{80.2}     % SA % better than random mean
\newcommand{\PoneSeedStd}{1.9}      % SA seed-to-seed std %
% ---------------------------------------------------------------------------
...
substation problem while particle-swarm trails at \Ppsogap\%. On the full
department-layout QAP, where exact optimization is intractable, a
Gilmore--Lawler lower bound together with a random-restart analysis brackets the
metaheuristics' quality. For storage, the Cube-per-Order-Index rule is shown to
be provably optimal and to save \PthreePopSave\% of travel over a realistic
...
expected for QAP-type problems.

Because no exact optimum is available at this scale, we characterize solution
quality three complementary ways. A Gilmore--Lawler lower
bound~\cite{lawler1963quadratic} certifies that the optimum is at least
\PoneGLB{}, placing SA within $\PoneGLBgap\%$ of optimal; the GL bound is
notoriously loose for the QAP, so this is a conservative certificate rather than
a tight one. Two further signals indicate the layout is genuinely strong: SA
improves on the mean of \num{5000} random feasible layouts by $\PoneRandImp\%$,
and its seed-to-seed standard deviation is only $\PoneSeedStd\%$ --- evidence of
a stable, near-optimal basin rather than a lucky single run.
Figure~\ref{fig:p1} shows the resulting layout and convergence (with the GL
bound and random reference marked).
...
substations built). Relative to a do-nothing ``build-everywhere'' policy --- a
substation in every zone, each self-serving, at cost \PtwoBuildAll{} --- the
optimized plan saves $\PtwoBuildSave\%$ by consolidating demand onto fewer
substations. The metaheuristics, run across 10 seeds, give best objectives of
\Pga{} (GA), \Psa{} (SA), and \Ppso{} (PSO), i.e.\ gaps of \Pgagap\% (GA/SA) and
\Ppsogap\% (PSO) to optimality. Particle-swarm, designed for continuous
...
travel relative to a random layout, and --- more meaningfully --- $\PthreePopSave\%$
relative to a realistic popularity (throughput-only) policy (\PthreePop) that
ignores product volume (Figure~\ref{fig:p3}).
